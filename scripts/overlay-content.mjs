...
// The Grad Life Blueprint is a separate, non-ministry project.
await rm("src/books/the-grad-life-blueprint.md", { force: true });
...
const forbiddenBranding = [
  "The Gleaning Ground",
  "<strong>The Gleaning</strong>"
];
const violations = [];

async function verifyBranding(directory) {
  if (!existsSync(directory)) return;
...
await verifyBranding("src");

if (violations.length) {
  throw new Error(`Old branding remains:\n${violations.join("\n")}`);
}
