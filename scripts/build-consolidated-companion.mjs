...
req(/    chapters\.forEach\(\(ch, i\) => \{\n      const y = 478 - i \* 43;[\s\S]*?    \}\);\n  \} else if \(index === 5\) \{/,`    text(cmd,"CHAPTER",52,500,7.5,"F2",GOLD);fitOneLine(cmd,"FORMATION WINDOW",365,500,87,7.5,"F2",GOLD,7,"right");
    chapters.forEach((ch,i)=>{const y=468-i*43;text(cmd,String(i+1).padStart(2,"0"),52,y,9.5,"F2",GOLD);fitOneLine(cmd,ch.title,88,y,270,9.3,"F2",NAVY,7.2,"left");fitOneLine(cmd,`Days ${i*10+1}-${i*10+10}`,365,y,87,8.3,"F1",MUTED,7.2,"right");line(cmd,52,y-10,PAGE_W-52,y-10,[.78,.75,.68],.35);});
...
req(/    chapters\.forEach\(\(ch, i\) => \{\n      const y = 332 - i \* 32;\n      text\(cmd, `\$\{ch\.number\}\. \$\{ch\.title\}`, 58, y \+ 7, 8\.5, "F1", INK\);\n      fieldArea\(cmd, fields, fillable, `chapter_\$\{ch\.number\}_completion_date`, 332, y, 120, 22, false\);\n    \}\);/,`    chapters.forEach((ch,i)=>{const y=332-i*32;fitOneLine(cmd,`${ch.number}. ${ch.title}`,58,y+7,258,8.5,"F1",INK,7.2,"left");fieldArea(cmd,fields,fillable,`chapter_${ch.number}_completion_date`,332,y,120,22,false);});`,"Journey Dates row alignment");
