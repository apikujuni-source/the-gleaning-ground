...
source=source.replaceAll("A 98-page, 90-day","An 89-page, 90-day").replaceAll("98 pages in a 7 × 10-inch","89 pages in a 7 × 10-inch").replaceAll("<strong>98</strong><span>Journal pages</span>","<strong>89</strong><span>Journal pages</span>").replaceAll("personal inventory, guided reflection","personal reflection").replaceAll("98 pages each","89 pages each").replaceAll("Receive your identity as God's child.","Understand your identity as God's child.").replaceAll("Let truth shape choices and boundaries.","Let truth shape your choices and boundaries.");
...
for(const path of [fillablePath,printPath]){const pdf=(await readFile(path)).toString("latin1");if(!pdf.includes("/Count 89"))throw new Error(`${path}: wrong page count`);if(!pdf.includes("www.gleaningground.com"))throw new Error(`${path}: license URL missing`);if(pdf.includes("PERSONAL INVENTORY")||pdf.includes("Guided Reflection"))throw new Error(`${path}: retired heading remains`);if((pdf.match(/PERSONAL REFLECTION/g)||[]).length!==9)throw new Error(`${path}: reflection count wrong`);for(const label of ["TRANSFORMATION","RESPONSIBILITY","MULTIPLICATION"])if(!pdf.includes(label))throw new Error(`${path}: expected formation label missing: ${label}`);for(const revised of ["Understand your identity as God's child.","Let truth shape your choices and boundaries."])if(!pdf.includes(revised))throw new Error(`${path}: revised objective missing: ${revised}`);for(const old of ["Receive your identity as God's child.","Let truth shape choices and boundaries."])if(pdf.includes(old))throw new Error(`${path}: old objective wording remains: ${old}`);}
console.log("Built and verified both 89-page Companion PDFs with width-aware wrapping, centered box text, fitted table labels, polished layout spacing, and updated objective wording.");
