...
function render(title, body, status = 200) {
  ...
  body{font-family:system-ui,-apple-system,sans-serif;background:#f6f3ec;color:#172536;margin:0}.wrap{max-width:1080px;margin:40px auto;padding:24px}h1{color:#173b62}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px}.card{background:#fff;border-radius:16px;padding:20px;box-shadow:0 8px 28px rgba(17,35,57,.07)}.big{font-size:1.65rem;font-weight:850;color:#173b62}.muted{color:#687586;font-size:.9rem}.status{display:inline-block;padding:4px 8px;border-radius:999px;background:#eef2f6;font-size:.78rem;font-weight:700}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:11px 8px;border-bottom:1px solid #edf0f3;font-size:.9rem;vertical-align:top}a{color:#173b62}.warn{background:#fff4df;padding:12px;border-radius:10px;margin:12px 0}.ok{background:#edf8f0;padding:12px;border-radius:10px;margin:12px 0}</style></head><body><div class="wrap">${body}</div></body></html>`, {
    status,
    headers: {
...
export default async (request) => {
  ...
  for (const session of sessions) {
    ...
    else if (status === 'available') add(currency, 'available', commission);
    ...
    } else if (status === 'pending') add(currency, 'pending', commission);
    ...
    rows.push({
      ...
      commission,
      status,
      payoutDate: String(m.commission_payout_date || ''),
      payoutReference: String(m.commission_payout_reference || ''),
      payoutMethod: String(m.commission_payout_method || '')
    });
  }
  ...
  const summary = Object.entries(totals).map(([currency, t]) => `
    ...
    <div class="card"><div class="muted">Available for payout</div><div class="big">${money(t.available,currency)}</div></div>
    <div class="card"><div class="muted">Paid</div><div class="big">${money(t.paid,currency)}</div></div>
    ${t.debt ? `<div class="card"><div class="muted">Refund offset due</div><div class="big">${money(t.debt,currency)}</div></div>` : ''}
  `).join('') || '<div class="card"><p>No attributed sales have been recorded yet.</p></div>';

  const history = rows.map((r) => `<tr><td>${new Date(r.created*1000).toLocaleDateString('en-US')}</td><td>${money(r.sale,r.currency)}</td><td>${money(r.commission,r.currency)}</td><td><span class="status">${esc(r.status)}</span></td><td>${r.payoutDate ? esc(r.payoutDate) : '—'}</td><td>${r.payoutReference ? `${esc(r.payoutMethod || 'Manual payout')}<br><span class="muted">${esc(r.payoutReference)}</span>` : '—'}</td></tr>`).join('');

  return render('Ambassador earnings', `<h1>Ambassador Earnings</h1><p>Hello <strong>${esc(ambassador.ambassadorName || '')}</strong>. Commission rate: <strong>${esc(ambassador.commissionRate)}%</strong>.</p><p class="muted">Referral ID: ${esc(ambassador.referralId)}</p><div class="grid">${summary}</div><div class="card" style="margin-top:16px"><h2>Payouts</h2><p>Commissions are tracked automatically. After the hold period and automatic refund checks, eligible commissions move to <strong>Available for payout</strong>. Payouts are currently completed manually by The Gleaning Ground and recorded here once sent.</p><p class="muted">You do not need to submit bank details on this dashboard.</p></div><div class="card" style="margin-top:16px"><h2>Sales history</h2>${history ? `<div style="overflow:auto"><table><thead><tr><th>Date</th><th>Sale</th><th>Commission</th><th>Status</th><th>Paid on</th><th>Payout reference</th></tr></thead><tbody>${history}</tbody></table></div>` : '<p>No sales yet.</p>'}<p class="muted">Commissions normally remain pending for 14 days to allow for refunds or payment reversals. Refunds after payout can be offset automatically against future commissions.</p></div>`);
};
